...
function CheckoutComponent() {
  ...
  return (
    <div className="min-h-screen bg-[#060504] text-zinc-100 flex items-center justify-center  p-6  relative selection:bg-amber-600 selection:text-black">
      <form
        onSubmit={handlePlaceOrder}
        className="w-full max-w-md bg-[#0B0A09] border border-[#1C1A17] p-8 rounded space-y-4 shadow-2xl relative"
      >
        <h2 className="text-sm uppercase tracking-[0.2em] text-[#C5A880] mb-2 font-serif">
          Luxury Checkout Panel
        </h2>
        ...
        {errorMessage && (
          <div className="bg-rose-950/20 border border-rose-900/40 p-3 rounded text-center">
            <p className="text-[10px] uppercase tracking-widest text-rose-500 font-medium animate-pulse">
              {errorMessage}
            </p>
          </div>
        )}
        ...
        <div>
          ...
          <input
            ...
            placeholder="e.g., Farzana Chowdhury"
        ...
        <div>
          ...
          <textarea
            ...
            onChange={handleInputChange}
            placeholder="House 12, Road 4, Gulshan-2, Dhaka"
            className="w-full bg-[#12110F] border border-[#1C1A17] p-3 text-xs rounded text-zinc-200 focus:outline-none focus:border-[#C5A880] h-20"
            required
          ></textarea>
        </div>
        ...
        <button
          type="submit"
          disabled={loading}
          className="w-full bg-[#C5A880] disabled:bg-zinc-800 text-black text-xs uppercase tracking-[0.2em] py-3.5 font-semibold hover:bg-[#b3956b] transition-all"
        >
      ...
      {showSuccessModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md z-50 flex items-center justify-center p-4">
          <div className="w-full max-w-sm bg-[#0B0A09] border border-[#1C1A17] p-8 rounded text-center relative overflow-hidden shadow-2xl animate-in fade-in zoom-in-95 duration-200">
            ...
            <h3 className="text-sm font-serif uppercase tracking-[0.2em] text-[#C5A880]">
              Order Authenticated
            </h3>
            <p className="text-[11px] text-zinc-400 tracking-wide mt-2 font-light">
              Your pieces have been successfully live-manifested via Cash On
              Delivery.
            </p>
            ...
            <div className="mt-4 bg-[#12110F] border border-[#1C1A17]/60 p-3 rounded">
              ...
              <span className="text-xs font-mono font-black text-amber-400 block mt-0.5 tracking-wider">
                {generatedTrackId}
              </span>
            </div>
